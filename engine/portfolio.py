...
class EnhancedPortfolio:
    ...
    def __init__(self, config_path: str = "config/settings.yaml"):
        ...
        # T+0 model: manage cash directly in portfolio (no deferred settlement)
        ...
        # Use deque for O(1) pops from left in FIFO consumption
        self.intraday_fifo: Dict[Tuple[str, str], deque] = defaultdict(deque)  # {(day_id, ticker): deque(BuyBlock)}
        ...
        # Informational counters (for audit purposes)
        self.total_operations: int = 0
        self.buy_operations: int = 0
        self.sell_operations: int = 0
    ...
    def _consume_intraday_fifo(self, ticker: str, day_id: str, sell_qty: int, sell_price: float, sell_trade_id: str) -> Dict:
        ...
        while remaining_sell_qty > 0 and buy_blocks:
            buy_block = buy_blocks[0]  # FIFO peek without removal
            
            if buy_block.remaining_qty <= 0:
                buy_blocks.popleft()  # Remove fully consumed block
                continue
    ...
    def buy(self, ticker: str, quantity: int, price: float, 
        ...
        try:
            ...
            # T+0: Check and debit cash immediately
            if self.cash < total_cost:
                logger.warning(
                    f"Insufficient cash for {ticker} buy: need R$ {total_cost:,.2f}, have R$ {self.cash:,.2f}"
                )
                return False
            ...
            # T+0: Debit cash immediately
            self.cash -= total_cost
            
            # Update tracking
            self.total_operations += 1
            self.buy_operations += 1
    ...
    def _calculate_accurate_win_rate(self) -> float:
        """
        Calculate accurate win rate based on SELL trades only.
        
        Returns:
            float: Win rate percentage (0-100)
        """
        sell_trades = [trade for trade in self.trade_history if trade.get('action') == 'SELL']
        if not sell_trades:
            return 0.0
        
        winning_sells = sum(1 for trade in sell_trades if trade.get('final_profit', 0) > 0)
        return (winning_sells / len(sell_trades)) * 100
    ...
    def get_portfolio_summary(self) -> Dict:
        ...
        buy_trades = [trade for trade in self.trade_history if trade.get('action') == 'BUY']
        sell_trades = [trade for trade in self.trade_history if trade.get('action') == 'SELL']
        
        # Calculate completed trades (for PnL calculation)
        completed_trades = len(sell_trades)  # Only SELL trades represent completed positions
        ...
        return {
            ...
            # Informational counters (for audit purposes)
            'total_operations': self.total_operations,    # BUY + SELL
            'buy_operations': self.buy_operations,        # Apenas BUYs
            'sell_operations': self.sell_operations,      # Apenas SELLs
            # SELL-based statistics (for user performance metrics)
            'total_trades': self.total_trades,            # Apenas SELLs
            'buy_trades': len(buy_trades),                # Legacy compatibility
            'sell_trades': len(sell_trades),              # Legacy compatibility
            'completed_trades': completed_trades,         # Alias for total_trades
            'winning_trades': self.winning_trades,
            'losing_trades': self.losing_trades,
            # Calculate win rate based on SELL trades only (where PnL is realized)
            'win_rate': self._calculate_accurate_win_rate(),
            ...
            # T+0 model: no settlement summary
    ...
    def export_audit_trails(self, base_path: str = "audit_trails") -> None:
        ...
        # T+0 model: no settlement audit trail to export
    ...
    def validate_sell_based_trade_counting(self) -> Dict[str, Any]:
        """
        Validate SELL-based trade counting consistency across all components.
        
        This method ensures that:
        1. total_trades equals the number of SELL operations
        2. All counting methodologies are consistent
        3. No double counting or missing counts exist
        ...
        try:
            ...
            buy_operations = len([t for t in self.trade_history if t.get('action') == 'BUY'])
            sell_operations = len([t for t in self.trade_history if t.get('action') == 'SELL'])
            
            # Validate core SELL-based principle
            sell_based_consistency = (self.total_trades == sell_operations)
            
            # Calculate basic statistics
            total_operations = buy_operations + sell_operations
            
            # Check for any trades without proper action classification
            unclassified_trades = len([t for t in self.trade_history if t.get('action') not in ['BUY', 'SELL']])
            
            # Compile validation results
            validation_results = {
                'is_consistent': sell_based_consistency and unclassified_trades == 0,
                'sell_based_principle': {
                    'portfolio_total_trades': self.total_trades,
                    'sell_operations_count': sell_operations,
                    'matches_expectation': sell_based_consistency
                },
                'operation_breakdown': {
                ...
                'validation_status': 'PASS' if sell_based_consistency and unclassified_trades == 0 else 'FAIL'
            }
            
            # Log validation results
            if validation_results['is_consistent']:
                logger.info(f"✅ SELL-based trade counting validation PASSED")
                logger.info(f"   Portfolio total_trades: {self.total_trades}")
                logger.info(f"   SELL operations: {sell_operations}")
                logger.info(f"   BUY operations: {buy_operations}")
            else:
                logger.warning(f"❌ SELL-based trade counting validation FAILED")
                logger.warning(f"   Portfolio total_trades: {self.total_trades}")
                logger.warning(f"   SELL operations: {sell_operations}")
                logger.warning(f"   Inconsistency detected!")
                
                if unclassified_trades > 0:
                    logger.warning(f"   Unclassified trades: {unclassified_trades}")
            
            return validation_results
            
        except Exception as e:
            logger.error(f"Error validating SELL-based trade counting: {e}")
            return {
                'is_consistent': False,
                'error': str(e),
...
def main():
    ...
    portfolio.buy(
        ...
        trade_type="swing_trade",
        trade_id="VALE3_001", 
        description="Initial VALE3 position"
    )
    ...
    portfolio.sell(
        ...
        trade_type="day_trade",
        trade_id="PETR4_002", 
        description="Close PETR4 day trade"
    )
    ...
    portfolio.sell(
        ...
        trade_type="swing_trade",
        trade_id="VALE3_002", 
        description="Close VALE3 position"
    )
    ...
    portfolio.buy(
        ...
        quantity=200, 
        price=25.0, 
        trade_date=base_date + timedelta(days=4), 
        trade_type="swing_trade",
        trade_id="ITUB4_001", 
        description="New position with loss carryforward"
    )
    ...
    portfolio.sell(
        ...
        trade_type="swing_trade",
        trade_id="ITUB4_002", 
        description="Close ITUB4 with loss offset"
    )
