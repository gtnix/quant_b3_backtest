...
pub async fn refresh_universe(
    ...
    info!("Universe 'top_volume' updated with {} symbols", all_stocks.len());
    Ok(())
}
...
pub async fn backfill(
    ...
    if symbols.is_empty() {
        warn!("No symbols in universe '{}'. Run refresh-universe first.", universe);
        return Ok(());
    }

    info!("Backfilling {} symbols with range '{}'", symbols.len(), range);
    ...
    for chunk in symbols.chunks(batch_size) {
        ...
        let (used, limit) = db.get_budget_status().await?;
        if used >= limit {
            error!("API quota exceeded. Processed {}/{} symbols", processed, symbols.len());
            return Err(IngestError::QuotaExceeded);
        }
        ...
        match brapi.fetch_quotes(&tickers, range, "1d").await {
            Ok((results, metrics)) => {
                db.log_request("/api/quote", &tickers, &metrics, None).await?;
                db.increment_budget().await?;
                ...
                for result in results {
                    ...
                    eprintln!("  {} - {} bars ({:?} to {:?})", 
                        result.symbol, bars.len(), first_date, last_date);
            ...
            Err(e) => {
                ...
                for ticker in &tickers {
                    db.update_ingestion_state(ticker, None, None, 0, Some(&e.to_string())).await?;
                }
            }
        }

        processed += chunk.len();
        eprintln!("\n>>> Progress: {}/{} symbols, {} total bars\n", processed, symbols.len(), total_bars);
        let _ = std::io::stderr().flush();
    }

    eprintln!("\n=== Backfill complete: {} symbols, {} bars ===", processed, total_bars);
    Ok(())
}
...
pub async fn update_incremental(
    ...
    let mut range_groups: std::collections::HashMap<&str, Vec<String>> = std::collections::HashMap::new();
    ...
    info!("Range groups: {:?}", range_groups.keys().collect::<Vec<_>>());

    let batch_size = brapi.max_tickers();
    let mut total_new_bars = 0;

    for (range, group_symbols) in range_groups {
        info!("Processing {} symbols with range '{}'", group_symbols.len(), range);
...
pub async fn verify_integrity(db: &Database) -> Result<(), IngestError> {
    ...
    if issues.is_empty() {
        ...
        for issue in &issues {
            warn!("  {} [{}]: {} occurrences", issue.symbol, issue.issue_type, issue.count);
        }
    }
...
pub async fn show_status(db: &Database) -> Result<(), IngestError> {
    ...
    println!("Date range:       {:?} to {:?}", stats.earliest_date, stats.latest_date);
    println!("Failed symbols:   {}", stats.failed_symbols);
    println!("\nAPI Budget:       {}/{} requests ({:.1}%)", 
        used, limit, (used as f64 / limit as f64) * 100.0);
    println!();
