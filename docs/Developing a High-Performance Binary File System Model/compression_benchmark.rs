// compression_benchmark.rs - Performance benchmarks for compression

use criterion::{black_box, criterion_group, criterion_main, Criterion, BenchmarkId, Throughput};
use obfs_prototype::CompressionPipeline;
...
fn generate_timeseries_data(num_points: usize) -> Vec<f32> {
    ...
    for i in 0..num_points {
        // Simulate realistic price movements with small changes
...
fn bench_compression_levels(c: &mut Criterion) {
    let data = generate_timeseries_data(1245); // 1245 trading days
    ...
    for level in [1, 3, 10, 19].iter() {
        ...
        group.bench_with_input(
            BenchmarkId::new("compress", level),
            level,
            |b, _| {
                b.iter(|| {
                    pipeline.compress(black_box(&data_bytes)).unwrap()
                });
            },
        );
    }
...
fn bench_decompression(c: &mut Criterion) {
    ...
    group.bench_function("decompress", |b| {
        b.iter(|| {
            pipeline.decompress(black_box(&compressed)).unwrap()
        });
    });
...
fn bench_compression_ratio(c: &mut Criterion) {
    ...
    for level in [1, 3, 10, 19].iter() {
        ...
        println!("Level {}: {} bytes -> {} bytes (ratio: {:.2}x)", 
            level, data_bytes.len(), compressed.len(), ratio);
    }
