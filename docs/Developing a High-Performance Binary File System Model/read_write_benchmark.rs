// read_write_benchmark.rs - Performance benchmarks for read/write operations

use criterion::{black_box, criterion_group, criterion_main, Criterion, BenchmarkId, Throughput};
use obfs_prototype::*;
use uuid::Uuid;

/// Create a test artifact for benchmarking
...
/// Benchmark artifact read performance
fn bench_read_artifact(c: &mut Criterion) {
    ...
    group.bench_function("read_single", |b| {
        ...
        b.iter(|| {
            reader.read_artifact(black_box(uuid)).unwrap()
        });
    });
...
/// Benchmark metrics-only read (optimized path)
fn bench_read_metrics_only(c: &mut Criterion) {
    ...
    group.bench_function("metrics_only", |b| {
        let reader = obfs.reader();
        b.iter(|| {
            reader.get_metrics(black_box(uuid)).unwrap()
        });
    });
