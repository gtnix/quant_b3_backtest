...
"""
    ...
    def save_processed_data(self, df: pd.DataFrame, series_id: int, start_date: str, end_date: str) -> bool:
        ...
        try:
            ...
            # Save to CSV
        # Avoid CSV emission in report flow
    ...
    def get_series_data(self, series_id: int, start_date: str, end_date: str, 
        ...
        if use_cache:
            ...
            if cached_data is not None:
                # Ensure daily_factor exists for SELIC data (series 11) even from cache
                if series_id == 11 and 'daily_factor' not in cached_data.columns:
                    cached_data['daily_factor'] = cached_data['valor'].apply(get_daily_factor)
                    logger.info("Added missing 'daily_factor' column to cached SELIC data.")
                return cached_data
        ...
        # --- Add daily_factor column for SELIC (series 11) BEFORE saving to cache ---
        ...
        # Save processed data (now includes daily_factor for SELIC)
        if save_processed:
    ...
    def preload_simulation_data(self, start_date: str, end_date: str) -> Dict[int, pd.DataFrame]:
        """
        Preload all SGS data for simulation to avoid per-bar API calls.
        Optimized version of get_all_series_data with enhanced logging.
        ...
        Returns:
            Dict[int, pd.DataFrame]: Dictionary mapping series_id to DataFrame
        """
        logger.info(f"Preloading SGS data for simulation: {start_date} to {end_date}")
        
        # Use existing method with optimized settings
        results = self.get_all_series_data(
            start_date=start_date, 
            end_date=end_date, 
            use_cache=True, 
            save_processed=True
        ...
        if results:
            logger.info(f"Successfully preloaded {len(results)} SGS series for simulation")
            for series_id, df in results.items():
                logger.info(f"  Series {series_id} ({self.SGS_SERIES[series_id]}): {len(df)} data points")
        else:
            logger.warning("No SGS data was preloaded for simulation")
    ...
    def save_consolidated_cache(self, series_id: int, df: pd.DataFrame) -> bool:
        """
        Save series data in consolidated parquet format for faster loading.
        Following IBOV caching patterns for optimal performance.
        
        Args:
            series_id (int): SGS series identifier
            df (pd.DataFrame): Consolidated data to save
        ...
        try:
            ...
            main_file = self.data_path / f"sgs_{series_id}_consolidated.parquet"
            
            # Save main parquet file
            df.to_parquet(main_file)
            
            # Save metadata (following IBOV pattern)
            metadata = {
                "series_id": series_id,
                "series_name": self.SGS_SERIES[series_id],
                "data_points": len(df),
                ...
                "format": "consolidated_parquet",
                "source": "sgs_api_cached"
            }
            
            metadata_file = main_file.with_suffix('.json')
            with open(metadata_file, 'w') as f:
                json.dump(metadata, f, indent=2)
            
            logger.info(f"Saved consolidated cache for series {series_id}: {main_file}")
            return True
            
        except Exception as e:
            logger.error(f"Error saving consolidated cache for series {series_id}: {e}")
            return False
    
    def load_consolidated_cache(self, series_id: int) -> Optional[pd.DataFrame]:
        """
        Load consolidated parquet cache if available.
        ...
        try:
            ...
            if main_file.exists():
                ...
                logger.info(f"Loaded consolidated cache for series {series_id}: {len(df)} data points")
                
                # Ensure daily_factor exists for SELIC data (series 11) even from consolidated cache
                if series_id == 11 and 'daily_factor' not in df.columns:
                    df['daily_factor'] = df['valor'].apply(get_daily_factor)
                    logger.info("Added missing 'daily_factor' column to consolidated SELIC cache.")
            ...
            logger.error(f"Error loading consolidated cache for series {series_id}: {e}")
            return None
    
    def consolidate_series_cache(self, series_id: int) -> bool:
        """
        Consolidate all CSV cache files for a series into a single parquet file.
        ...
        """
        try:
            logger.info(f"Consolidating cache for series {series_id} ({self.SGS_SERIES[series_id]})")
            
            # Find all CSV files for this series
            csv_files = list(self.data_path.glob(f"sgs_{series_id}_*.csv"))
            
            if not csv_files:
                logger.warning(f"No CSV cache files found for series {series_id}")
                return False
            
            logger.info(f"Found {len(csv_files)} CSV cache files for series {series_id}")
            ...
            if not all_data:
                logger.error(f"No valid data found for series {series_id}")
                return False
            
            # Combine and sort data
            consolidated_df = pd.concat(all_data, axis=0)
            ...
            # Ensure daily_factor exists for SELIC data (series 11) in consolidated cache
            if series_id == 11 and 'daily_factor' not in consolidated_df.columns:
                consolidated_df['daily_factor'] = consolidated_df['valor'].apply(get_daily_factor)
                logger.info("Added 'daily_factor' column to consolidated SELIC data.")
            
            logger.info(f"Consolidated {len(consolidated_df)} unique data points for series {series_id}")
            logger.info(f"Date range: {consolidated_df.index.min()} to {consolidated_df.index.max()}")
            
            # Save consolidated cache
            success = self.save_consolidated_cache(series_id, consolidated_df)
            
            if success:
                logger.info(f"Successfully created consolidated parquet cache for series {series_id}")
            else:
                logger.error(f"Failed to save consolidated cache for series {series_id}")
            
            return success
            
        except Exception as e:
            logger.error(f"Error consolidating cache for series {series_id}: {e}")
            return False
    
    def consolidate_existing_cache(self) -> Dict[str, bool]:
        """
        Consolidate all existing CSV cache files to parquet format for better performance.
        
        Returns:
            Dict[str, bool]: Results for each series consolidation
        """
        logger.info("Starting SGS cache consolidation to parquet format...")
        
        # Check existing cache
        csv_files = self.get_available_processed_files()
        logger.info(f"Found {len(csv_files)} existing CSV cache files")
        
        if not csv_files:
            logger.warning("No existing CSV cache files found. Run some backtests first to generate cache.")
            return {}
        
        # Consolidate cache for each series
        results = {}
        success_count = 0
        
        for series_id in self.SGS_SERIES.keys():
            success = self.consolidate_series_cache(series_id)
            results[f"series_{series_id}"] = success
            if success:
                success_count += 1
        
        logger.info(f"Cache consolidation completed: {success_count}/{len(self.SGS_SERIES)} series optimized")
        
        # Show performance improvement estimate
        if success_count > 0:
            logger.info("Performance improvement: Simulation SGS data loading will be significantly faster")
            logger.info("Note: Existing CSV cache files are preserved for backward compatibility")
        
        return results

    def regenerate_selic_cache_with_daily_factor(self) -> Dict[str, bool]:
        """
        Regenerate existing SELIC cache files to include daily_factor column.
        This fixes existing cache files that were created before the daily_factor fix.
        
        Returns:
            Dict[str, bool]: Results for each cache file regeneration
        """
        logger.info("Starting SELIC cache regeneration to add daily_factor column...")
        
        # Find all SELIC CSV cache files
        selic_csv_files = list(self.data_path.glob("sgs_11_*.csv"))
        
        if not selic_csv_files:
            logger.warning("No SELIC cache files found to regenerate")
            return {}
        
        logger.info(f"Found {len(selic_csv_files)} SELIC cache files to regenerate")
        
        results = {}
        success_count = 0
        
        for csv_file in selic_csv_files:
            try:
                logger.info(f"Regenerating {csv_file.name}")
                
                # Load the existing cache file
                df = pd.read_csv(csv_file, index_col=0, parse_dates=True)
                
                # Check if daily_factor already exists
                if 'daily_factor' in df.columns:
                    logger.info(f"  {csv_file.name} already has daily_factor column, skipping")
                    results[csv_file.name] = True
                    success_count += 1
                    continue
                
                # Add daily_factor column
                df['daily_factor'] = df['valor'].apply(get_daily_factor)
                
                # Save the updated file
        # Avoid CSV emission in report flow
                
                logger.info(f"  Successfully added daily_factor to {csv_file.name}")
                results[csv_file.name] = True
                success_count += 1
                
            except Exception as e:
                logger.error(f"  Failed to regenerate {csv_file.name}: {e}")
                results[csv_file.name] = False
        
        # Also regenerate consolidated parquet if it exists
        consolidated_file = self.data_path / "sgs_11_consolidated.parquet"
        if consolidated_file.exists():
            try:
                logger.info("Regenerating consolidated SELIC parquet cache")
                
                # Force regeneration of consolidated cache with daily_factor
                success = self.consolidate_series_cache(11)
                results["sgs_11_consolidated.parquet"] = success
                
                if success:
                    logger.info("Successfully regenerated consolidated SELIC cache with daily_factor")
                    success_count += 1
                else:
                    logger.error("Failed to regenerate consolidated SELIC cache")
                    
            except Exception as e:
                logger.error(f"Error regenerating consolidated SELIC cache: {e}")
                results["sgs_11_consolidated.parquet"] = False
        
        logger.info(f"SELIC cache regeneration completed: {success_count}/{len(results)} files updated")
        
        if success_count > 0:
            logger.info("✅ SELIC cache regeneration successful! All cache files now include daily_factor.")
            logger.info("🔧 The 'daily_factor' error should now be resolved.")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='SGS Data Loader with Cache Optimization')
    parser.add_argument('--consolidate', action='store_true', 
                       help='Consolidate existing CSV cache to parquet format')
    parser.add_argument('--regenerate-selic', action='store_true',
                       help='Regenerate existing SELIC cache files with daily_factor column')
    ...
    if args.consolidate:
        # Run cache consolidation (quiet)
        _ = loader.consolidate_existing_cache()
        return
    
    if args.regenerate_selic:
        # Run SELIC cache regeneration (quiet)
        _ = loader.regenerate_selic_cache_with_daily_factor()
        return
